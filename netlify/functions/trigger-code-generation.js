...
exports.handler = async (event, context) => {
  console.log('[MANUAL TRIGGER] Starting manual code generation...');
  ...
  try {
    // Fetch all 'generating' batches and filter in JavaScript
    // (Supabase JS client doesn't support column-to-column comparison)
    ...
    if (fetchError) {
      console.error('[MANUAL] Error fetching batches:', fetchError.message);
      return {
        statusCode: 500,
        headers,
        body: JSON.stringify({ error: 'Failed to fetch incomplete batches', details: fetchError.message })
      };
    }

    if (!allBatches || allBatches.length === 0) {
      console.log('[MANUAL] No incomplete batches found.');
      return {
        statusCode: 200,
        headers,
    ...
    if (incompleteBatches.length === 0) {
      console.log('[MANUAL] All generating batches are complete.');
      return {
        statusCode: 200,
        headers,
        body: JSON.stringify({ message: 'All batches complete', processed: 0 })
      };
    }

    console.log(`[MANUAL] Found ${incompleteBatches.length} incomplete batch(es)`);
    ...
    for (const batch of incompleteBatches) {
      ...
      console.log(`[MANUAL] Batch ${batch.batch_id}: Generating ${toGenerate} codes (${batch.codes_generated}/${batch.quantity})`);
      ...
      try {
        ...
        console.log(`[MANUAL] Batch ${batch.batch_id}: ${inserted} codes generated. Total: ${newTotal}/${batch.quantity} ${isComplete ? '✓ COMPLETE' : ''}`);

      } catch (err) {
        console.error(`[MANUAL] Error processing batch ${batch.batch_id}:`, err.message);
        results.push({
          batch_id: batch.batch_id,
          error: err.message
        });
    ...
    console.log(`[MANUAL] Completed in ${elapsed}s. Total codes generated: ${totalGenerated}`);
    ...
    console.error('[MANUAL] Fatal error:', error.message);
    return {
      statusCode: 500,
      headers,
      body: JSON.stringify({ 
