...
<head>
  ...
  <script>
    // Fire as soon as DOM is ready — do NOT wait for images to load.
    // For large batches (50,000+ codes) images may still be loading but
    // the document structure and text codes are fully ready to print.
    document.addEventListener('DOMContentLoaded', function () {
      var bar = document.getElementById('qc-status-bar');
      var btns = document.querySelectorAll('.qc-action-btn');
      if (bar) bar.textContent = '✅ Document ready — click Print or Save as PDF';
...
<body>
  <div class="no-print">
    ...
    <p id="qc-status-bar" style="margin-bottom: 12px; font-size: 12px; color: #f59e0b;">⏳ Preparing document, please wait...</p>
    <button class="qc-action-btn" onclick="printPage()" disabled style="opacity:0.5; cursor:not-allowed;">🖨️ Print Document</button>
    <button class="qc-action-btn" onclick="savePDF()" disabled style="opacity:0.5; cursor:not-allowed;">💾 Save as PDF</button>
      ...
      <div class="codes-grid">
        ${pageCodes.map((code) => `
          <div class="code-card">
            ...
            ${code.qr_code_url ? `
              <div class="qr-code">
                <img src="${code.qr_code_url}" alt="QR" loading="lazy" onerror="this.style.display='none'" />
              </div>
            ` : ''}
          </div>
        `).join('')}
...
exports.handler = async (event, context) => {
  ...
  try {
    ...
    // Upload to Supabase Storage — bypasses Netlify's 6MB response limit
    console.log('Uploading document to Supabase Storage...');
    const fileName = `batch-${batchId}-${Date.now()}.html`;

    const { error: uploadError } = await supabase.storage
      .from('batch-documents')
      .upload(fileName, Buffer.from(htmlContent, 'utf8'), {
    ...
    console.log('Document ready. Returning signed URL.');

    // Return just the URL — tiny JSON response, no size limit issues
    return {
      statusCode: 200,
      headers: {
        'Content-Type': 'application/json',
