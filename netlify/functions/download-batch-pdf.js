...
// Generate pages with 80 codes each (5 columns x 16 rows) - NO serial numbers, NO status
...
exports.handler = async (event, context) => {
  ...
  try {
    ...
    // ── Upload HTML to Supabase Storage (bypasses 6MB response limit) ──────
    ...
    // Return just the URL — tiny response, no size limit issues
    return {
      statusCode: 200,
      headers: {
        'Content-Type': 'application/json',
