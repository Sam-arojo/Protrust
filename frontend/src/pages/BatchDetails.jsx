...
function BatchDetails() {
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      ...
      // Step 1: Call the function — get back a signed Supabase URL
      ...
      // Step 2: Fetch the actual HTML content from the Supabase signed URL
      const htmlResponse = await fetch(data.downloadUrl);
      const htmlContent = await htmlResponse.text();

      // Step 3: Create a Blob URL with the correct HTML content type
      // This makes the browser render it as a proper webpage, not raw text
      const blob = new Blob([htmlContent], { type: 'text/html' });
      const blobUrl = URL.createObjectURL(blob);

      // Step 4: Open blob URL in a new tab — renders fully as a webpage
      const printWindow = window.open(blobUrl, '_blank');

      // Step 5: Clean up the blob URL after the window loads
      if (printWindow) {
